...
function write_data($source, $column, $label)
{
  ...
  foreach ($next->{'results'} as &$value) {
	   $rank++;
	   echo $value->{$column}."\n".str_replace('"', "", str_replace("'","",$value->{$column}))."\n";
	   $query = "INSERT INTO topservatory VALUES ('".$label."', '".$time."', '', ".$rank.", '".str_replace("'","",$value->{$column})."')";
	   echo $query;
...
//print_r(get_data($wikipedia));

//iterate through all data and put in db

//mysql_query("SELECT * FROM topservatory") or die(mysql_error());
